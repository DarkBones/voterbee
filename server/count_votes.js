...
const countVotes = (candidates, votesInitial, results = null, round = 0) => {
  if (round > 1000) {
    return results
  }
...
const processVotes = async (electionId, userId) => {
  ...
  return await electionRef.get()
    .then(async (snapshot) => {
      const election = snapshot.val()
      if (!election) {
        return Promise.reject({ message: 'Election not found', status: 404 })
      }
